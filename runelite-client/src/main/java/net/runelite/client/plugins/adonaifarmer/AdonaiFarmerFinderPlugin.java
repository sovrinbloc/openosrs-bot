...
//		}
		...
		{
			log.info("state != PlayerState.DROPPING");
			drop();
		}
	}
	...
	{
		log.info("Inside dropItemsExcept");
		...
		log.info("Seed box == Null: {}", seedBox == null);
		log.info("Seed box Bounds = {}", Objects.requireNonNull(seedBox).getBounds());
		...
		log.info("Seed box click point = {}", randomClickPoint);
		log.info("Seed box SCREEN click point = {}", clickPoint);
		doClick(clickPoint);
	...
	private void doClick(Point clickPoint)
	{
		...
		{
			log.info("clicked: {}", sendClick(clickPoint));
			try
			{
				mutex.lock();
				--dropCount;
	...
	private boolean sendClick(Point clickPoint)
	{
		try
		{
