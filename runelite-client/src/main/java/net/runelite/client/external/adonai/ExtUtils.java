...
public class ExtUtils<operator, T2>
{
	...
	public static class Inventory
	{
		...
		{
			log.info("Inside getItemsExceptById");
			ItemContainer       itemContainer = client.getItemContainer(InventoryID.INVENTORY);
			...
			log.info("Leaving getItemsExceptById");
			return iItems;
		}
		...
		public static InventoryItem getItem(int itemId)
		{
			...
			final Item[]                       items          = itemContainer.getItems();
			InventoryItem[] inventoryItems = Inventory.convertToInventoryItems(items);
			log.info("[DEBUG]: After converting to inventory item. " + Arrays.toString(inventoryItems));
